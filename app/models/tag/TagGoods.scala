...
object TagGoodses extends Table[TagGoods]("tag_goods") {
  ...
  def * = id.? ~ cid ~ tagName  ~ goodsId ~ addNum ~ checkState ~ sortNum  <>(TagGoods, TagGoods.unapply _)
  def autoInc  =id.? ~ cid ~ tagName  ~ goodsId ~ addNum ~ checkState ~ sortNum  <>(TagGoods, TagGoods.unapply _) returning id

  def autoInc2 = tagName  ~ goodsId ~ cid  returning id
  ...
  def insert(tagName:String,goodsId:Long,cid:Int)(implicit  session:Session)={
    TagGoodses.autoInc2.insert(tagName,goodsId,cid)
  }
