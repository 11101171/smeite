...
object GoodsDao {
 ...
 def modifyIsMember(goodsId:Long,isMember:Boolean) =  database.withSession {  implicit session:Session =>
  ...
  /* 修改价格*/
  def updateTaobaoke(numIid:Long,name:String,volume:Int,price:String,promotionPrice:String) = database.withSession {  implicit session:Session =>
    (for (c<-Goodses if c.numIid === numIid )yield c.name~c.volume ~ c.price ~c.promotionPrice ~ c.collectTime).update((name,volume,price,promotionPrice,new Timestamp(System.currentTimeMillis())))
  }
