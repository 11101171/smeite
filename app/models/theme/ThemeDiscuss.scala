...
object ThemeDiscusses  extends  Table[ThemeDiscuss]("theme_discuss") {
  ...
  def * =id.? ~ themeId ~ uid ~ uname  ~ quoteContent.? ~ content ~ checkState ~ addTime.?   <>(ThemeDiscuss, ThemeDiscuss.unapply _)
  def autoInc =id.? ~ themeId ~ uid ~ uname  ~ quoteContent.?  ~ content ~ checkState ~ addTime.?   <>(ThemeDiscuss, ThemeDiscuss.unapply _) returning id
