...
object API extends Controller {
  ...
  * */ 
  ...
  /* 淘宝客 convert*/
  def convertProduct(numIid:Long) =Action{
    val goods =GoodsDao.find(numIid)
    if(goods.isEmpty) Ok(views.html.goods.nofound())
      ...
      Ok(views.html.ugc.api.convertProduct(goods.get)).withCookies(Cookie("timestamp",timestamp,httpOnly=false),Cookie("sign", sign,httpOnly=false))
